...
class BatchSheetForm(forms.Form):
    ...
    packaging_requirements = forms.CharField(label="Packaging Requirements", max_length=100, required=False)
    batch_amount = forms.DecimalField(label="Batch amount", 
                                      min_value=0, 
                                      max_digits=9, 
                                      decimal_places=3, required=False)
    # update_rm_inventory = forms.BooleanField(label="Update Inventory",required=False)
