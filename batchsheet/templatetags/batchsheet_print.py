...
def batchsheet_print(lot_pk):
    ...
    if qv != True:
        flavor = "%s -- NOT APPROVED -- %s" % (flavor.__str__(), qv)
        #c = Context({'flavor':u"%s -- NOT APPROVED -- %s" % (flavor.__str__(), qv)})
        #json_dict['batchsheet'] = loader.get_template('batchsheet/batchsheet_print.html').render(c)
    elif len(dci) != 0:
        flavor = "%s -- NOT APPROVED -- Contains discontinued ingredients: %s" % (flavor.__str__(), ", ".join(dci))
        #c = Context({'flavor':u"%s -- NOT APPROVED -- Contains discontinued ingredients: %s" % (flavor.__str__(), ", ".join(dci))})
        #json_dict['batchsheet'] = loader.get_template('batchsheet/batchsheet_print.html').render(c)
    elif flavor.approved == False:
        flavor = "%s -- NOT APPROVED" % flavor.__str__()
        #c = Context({'flavor':u"%s -- NOT APPROVED" % flavor.__str__()})
        #json_dict['batchsheet'] = loader.get_template('batchsheet/batchsheet_print.html').render(c)
    else:
        flavor = lot.flavor
        '''
