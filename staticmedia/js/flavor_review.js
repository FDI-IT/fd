...
function print_experimental_review(experimental_number) {
    ...
    newwindow[newwindow.addEventListener ? 'addEventListener' : 'attachEvent'](
        (newwindow.attachEvent ? 'on' : '') + 'load', function(e){
            ...
            if ( new_table.is("table") ) {
                div_flavor_tabs.innerHTML = '<caption>Batch Weight: ' + batch_weight + '</caption>' + new_table.prop('outerHTML');
            } else {
                new_table.find('table');
                div_flavor_tabs.innerHTML = '<caption>Batch Weight: ' + batch_weight + '</caption>' + new_table.prop('outerHTML');
            }
            newwindow.print();
        }, false
...
function print_review(flavor_number) {
    ...
    newwindow[newwindow.addEventListener ? 'addEventListener' : 'attachEvent'](
        (newwindow.attachEvent ? 'on' : '') + 'load', function(e){
            ...
            // var batch_weight = jQuery('#adjusted_weight').val();
            var batch_weight = "";
            var id = visible_data.closest("div").prop("id");
            if(id == "formula"){
              batch_weight = jQuery('#adjusted_weight').val();
            }
            else{
              batch_weight = jQuery('#actual_adjusted_weight').val();
            }
            // console.log(id);
            if (!batch_weight) {
                batch_weight = 1000;
	...
	// old, firefox exclusive onload function
	// newwindow.onload = function (e) {
		// var div_flavor_tabs = newwindow.document.getElementById('product_tabs');
		// var new_table = copy_table.clone();
		// var batch_weight = jQuery('#adjusted_weight').val();
...
function spec_sheet(flavor_number) {
	...
	// old, onload on a new window doesn't work in IE
	// newwindow.onload = function (e) {
		// newwindow.print();
	// };
}
