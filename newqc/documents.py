...
class ImportBCDoc():
    ...
    def generate_scanneddoc_kwargs(self):
        ...
        if self.timestamp: #if timestamp exists, we know it's a testcard and can add the qc time field
            context_dict['qc_time'] = datetime.fromtimestamp(float(self.timestamp))
        return context_dict
        ...
        for s in self.symbol_list:
            ...
            if len(bc_split) == 2: #if the length is 2, there's no timestamp (type is not RETAIN)
                if bc_split[0] in type_map:
                    bc_split.append(None) #append None because there is no timestamp
                    return bc_split
