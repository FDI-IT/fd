...
def parse_boxscore_index(index_url):
    doc = html5lib.parse(urllib.request.urlopen(index_url), treebuilder="dom")
    tables = doc.getElementsByTagName('table')
    index_urls = []
    extract_clean_file_name = re.compile(r"years\/(\d{4})(_AFL|_AAFC){0,1}/games")
    for t in tables:
        for l in t.getElementsByTagName('a'):
            index_urls.append("%s%s" % (domain, list(l.attributes.values())[0].value))
...
def get_box_score(box_score_url):
    ...
    p = urllib.request.urlopen(box_score_url)
    f.write(p.read())
    f.close()
