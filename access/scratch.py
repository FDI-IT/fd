...
import operator
from access.models import Flavor, Ingredient, Formula, FormulaTree, LeafWeight, Solvent, IndivisibleLeafWeight, FormulaException, IngredientStatement, DIACETYL_PKS, PG_PKS, SOLVENT_NAMES, KosherGroup
...
def synchronize_price(f, verbose=False):
    ...
    for g in f.gazinta.all():
        ...
        g.purchase_price_update = lastspdate
        g.save()
...
def set_allergen_bools_from_text_field():
    for i in Ingredient.objects.all():
        if i.allergen in aller_dict:
            allergen_value = aller_dict[i.allergen]
            mutated = False
...
def parse_ingredient_allergens():
    ...
    cursor.execute("UPDATE access_integratedproduct SET crustacean='', eggs=False, fish='', milk=False, peanuts=False, soybeans=false, treenuts='', wheat=false, sunflower=false, sesame=false,mollusks=false,mustard=false,celery=false,lupines=false,yellow_5=false,allergen='NONE';")
    allergenic_flavors = []
        ...
        for allergen in Ingredient.boolean_allergens + Ingredient.text_allergens:
            if getattr(allergenic_flavor, allergen):
                if allergen in Ingredient.boolean_allergens:
                    ...
                    flavor_allergens.append('%s (%s)' % (allergen, getattr(allergenic_flavor, allergen)))
    ...
    #     for allergen in aller_attrs:
    #         if getattr(allergenic_flavor, allergen):
    #             flavor_allergens.append(Ingredient._meta.get_field(allergen).verbose_name)
    #     allergenic_flavor.allergen = "Yes: %s" % ','.join(flavor_allergens)
    #     allergenic_flavor.save()

@transaction.atomic
def parse_single_rm_allergen(i):
    i.allergen = i.get_allergen_text()
    i.save()

@transaction.atomic
def parse_rm_allergens():
    for i in Ingredient.objects.all():
        parse_single_rm_allergen(i)
...
#         my_valid = False
    ...
    #set all allergens to False or '' before recalculating them
    for allergen in Ingredient.boolean_allergens:
        setattr(flavor, allergen, False)
    for allergen in Ingredient.text_allergens:
        setattr(flavor, allergen, '')
        ...
        for allergen in Ingredient.boolean_allergens + Ingredient.text_allergens:
            if getattr(lw.ingredient, allergen):
                ...
                if allergen in Ingredient.boolean_allergens:
                    ...
                    setattr(flavor, allergen, True)
                elif allergen in Ingredient.text_allergens:
                    flavor.update_text_allergen(allergen, getattr(lw.ingredient, allergen))
                    #allergens.add('%s (%s)' % (allergen, getattr(lw.ingredient, allergen)))
    ...
    if has_allergens:
        ...
        flavor.allergen = flavor.get_allergen_text()
        flavor.ccp2 = True
        flavor.ccp4 = True
        # justink: allergen fix will update flavor allergen fields and not just the allergen text fields
        # for a in allergens:
        #     if a in Ingredient.boolean_allergens:
        #         setattr(flavor, a, True)
        #
        #     else:
        #     # allergen is a text allergen and have to split the allergen by field and value
        #         temp = a.split(" ")
        #         setattr(flavor, temp[0], (getattr(flavor, temp[0]) + temp[1].strip('(').strip(')') + ' ' ))
    ...
    #ffffflavor.calculate_gmo()

    reversion.set_comment("Recalculated")
...
# def get_ingredient_statements_from_spec_sheets():
#     #for now, we won't delete already existing ingredient statements
#     #only create them for products that don't already have them
#     for ssi in SpecSheetInfo.objects.all():
#         if not IngredientStatement.objects.filter(flavor=ssi.flavor).exists():
#             ingredient_statement = IngredientStatement(flavor=ssi.flavor)
#
#             #if the spec sheet has no ingredient statement, we don't want to create an IngredientStatement object
#             if ssi.ingredient_statement == None or ssi.ingredient_statement == '':
#                 pass
#             else:
#                 #only verify the ingredient statement if:
#                 #1. the spec sheet is dated after 1/1/2014
#                 #AND 2. the product has been QC'd in the past two years
#                 ingredient_statement.ingredient_statement = ssi.ingredient_statement
#                 if ssi.date == None:
#                     ingredient_statement.verified = False
#                 else:
#                     twoyearsago = datetime.now().date() - relativedelta(years=2)
#                     if Retain.objects.filter(lot__flavor__in=ssi.flavor.loaded_renumber_list).filter(date__gte=twoyearsago).exists() and ssi.date > date(2014, 01, 01):
