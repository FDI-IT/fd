...
from access.models import Flavor, Ingredient, FormulaException, Formula
...
def test_aggregator():
    flavors_to_validate_list = Flavor.objects.annotate(tw=Sum('formula__amount')).filter(tw=1000).values_list('id',flat=True)
    formula_dict = {}
    for k,g in groupby(Formula.objects.filter(flavor__id__in=flavors_to_validate_list), xfunc):
        formula_dict[k] = list(g)
...
class IntegrityCheck:
    ...
    def __init__(self,debug=True):
        self.debug = debug
        self.flavors_to_validate_list = Flavor.objects.annotate(tw=Sum('formula__amount')).filter(tw=1000).values_list('id',flat=True)
        self.formula_dict = {}
        for k,g in groupby(Formula.objects.filter(flavor__id__in=self.flavors_to_validate_list), xfunc):
            self.formula_dict[k] = list(g)
    ...
    def close_queue(self):
        ...
        for f,v in self.validated_flavors.iteritems():
            validation_status, rmc = v
            #f.valid = 
...
class ComplexIngredientValidator():
    ...
    """
    def validate(self):
        complex_ingredients = Ingredient.objects.filter(flavornum__isnull=False)
        for ing in complex_ingredients:
            try:
                f = Flavor.objects.get(number=ing.flavornum)
                ing.product_name = f.__unicode__()[:60]
                ing.save()
