...
def parse_numbers(new_experimentals_file):
    ...
    for new_experimental_row in read_experimental_rows(new_experimentals_file):
        ...
        if type(parsed_data) is unicode:
            print parsed_data
        else:
            parsed_data.append(retain_number)
            labels_csv_writer.writerow(map(unicode,parsed_data))            
            retain_number += 1
...
def parse_input_row(input_row):
    ...
    try:
        ...
        return u"%s - Unable to find experimental number." % experimental_num
        
    if initials.upper() != el.initials.upper():
        return u"%s - Unable to match initials. %s %s" % (experimental_num, initials, el.initials)
